...
public static class UpdateSession
{
    ...
    public sealed class Endpoint : IEndpoint
    {
        ...
        {
            app.MapPut("sessions/{id}", Handler)
                .WithTags("Sessions")
                .Produces<Response>()
    ...
    public static async Task<IResult> Handler(int id, Request request, AppDbContext context,
        ...
        ClaimsPrincipal user, IValidator<Request> validator)
    {
        ...
        session.MidrangeAttempts = request.MidrangeAttempts;
        session.ThreePointMakes = request.ThreePointMakes;
        session.ThreePointAttempts = request.ThreePointAttempts;
        session.FreeThrowMakes = request.FreeThrowMakes;
        session.FreeThrowAttempts = request.FreeThrowAttempts;
        session.Drills =
        [
        ...
        return TypedResults.Ok(new Response(
            ...
            OverallShotPercentage: (session.PaintAttempts + session.MidrangeAttempts + session.ThreePointAttempts +
                ...
                : 0,
            Drills: [.. session.Drills.Select(d => new DrillResponse(d.Id, d.Name, d.CompletionTimeInSeconds))]
        ));
    }
}